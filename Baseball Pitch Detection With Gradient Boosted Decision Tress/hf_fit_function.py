...
json_filepath = '../MLSPdata/MixedJsons/'
...
def plot_check(factors, xs, ys, filename):
    ...
    plt.title(filename)
    # plt.show()
    plt.savefig('../MLSPdata/fitplt/' + filename.split('.')[0] + '.png')
    plt.clf()
...
def main():
    ...
    np.savetxt("../MLSPdata/train_data.csv", train_data, delimiter=",")
    np.savetxt("../MLSPdata/train_label.csv", train_label, delimiter=",")
