...
class RNN_Phoneme_Classifier(object):
    ...
    def forward(self, x, h_0=None):
        ...
        for s in range(0,seq_len):
            this_seq=x[:,s,:]
            for cell in range (0,len(self.rnn)):
                this_cell=self.rnn[cell]
                this_hidden=hidden[cell]
                if cell==0:
                    out=this_cell.forward(this_seq,this_hidden)
                else:
                    out=this_cell.forward(hidden[cell-1],this_hidden)
                #print(out.shape)
                hidden[cell]=out
            self.hiddens.append(hidden.copy())

        return self.output_layer.forward(self.hiddens[-1][-1])
    ...
    def backward(self, delta):
        ...
        * Normalize dh by batch_size since initial hidden states are also treated
        ...
        for t in range(seq_len-1,-1,-1):
            this_h_time=self.hiddens[t+1]
            last_h_time=self.hiddens[t]
            for layer in range(self.num_layers-1,-1,-1):
                if layer==0:
                    h_prev_l=self.x[:,t,:]
                else:
                    h_prev_l=this_h_time[layer-1]
                   # print(h_prev_l.shape)
                h=this_h_time[layer]
                h_prev_t=last_h_time[layer]
                if layer == self.num_layers-1:
                    delta_h = dh[layer]
                    new_dx,new_dh=self.rnn[layer].backward(delta_h,h,h_prev_l,h_prev_t)
                    dh[layer]= new_dh
                else:
                    #print(layer)
                    delta_h = dh[layer]+new_dx
                    #print(new_dh.shape,delta_h.shape)
                    new_dx, new_dh = self.rnn[layer].backward(delta_h, h, h_prev_l, h_prev_t)
                    dh[layer] = new_dh
