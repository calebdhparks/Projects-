...
void gen_graph(Node *node, int count, vector<string> names){	
	if(node->best_feature>=0){
	ofstream outfile("dtree.gv", std::ios_base::app);
	outfile<<count<<" [label=\""<<names[node->best_feature]<<" <= "<<node->split_val
		<<"\\ error = "<<node->best_err<<"\\ nsamples = "<<node->members.size()<<"\"] ; \n";
	outfile.close();
	}
	else{
	 ofstream outfile("dtree.gv", std::ios_base::app);
        outfile<<count<<" [label=\" nsamples = "<<node->members.size()<<"\"] ; \n";
        outfile.close();
	}
	if(!node->children.empty()){
	gen_graph(&node->children[0],count+1,names);
	gen_graph(&node->children[1],count+1,names);
	}
}
void update_graph(){
	fstream graphfile ("dtree.gv");
        ofstream outfile3 ("dtree_.gv");
        int num=0;
        while(!graphfile.eof()){
        string line;
        getline(graphfile,line);
        if(num>=2&&line.length()>1){
                int count = num-2;
                string scount=to_string(count);
                scount.append(" ");
                line.insert(0,scount);
                }
        outfile3<<line<<"\n";
        num++;
	}
}
void finish_graph(int depth){
	fstream graphfile ("dtree_.gv");
        ofstream outfile ("dtree_final.gv");
	int numLines=0;
	string line;
        string num , depth_string;
	int current,parent;
	while(!graphfile.eof()){
	getline(graphfile,line);
        numLines++;
	...
	last[0]="0";
	last[1]="1";
	for(int x =0;x<numLines-6;x++){
	getline(graphfile,num,' ');
	getline(graphfile,depth_string,' ');	
	getline(graphfile,line);
	outfile<<num<<" "<<line<<"\n";
	current=std::atoi(depth_string.c_str());
	if(current>0){
		//cout<<(current)<<endl;
		parent=current-1;
		last[current]=num;
		outfile<<last[parent]<<" -> "<<num<<";\n";
		}
	}
	outfile<<"}";
	outfile.close();
	remove("dtree.gv");
	remove("dtree_.gv");
	rename("dtree_final.gv","dtree.gv");
}
int main(int argc, char *argv[]) {
	...
	for(int x=0;x<=15;x++){
	getline(infile,featNames,',');
	names.push_back(featNames);
	}
	infile.close();
	gen_graph(decision_tree.root,0,names);
	update_graph();
	finish_graph(depth);	
	return 0;
}
